...
%% kL - Loss PLOTS    
    ...
    hold on
    for j = (1:numel(kL_factor_s))
        plot(alpha, ER_kL1(j,:), DisplayName="kL Factor = " + kL_factor_s(j))
    end
    hold off
    legend
    grid on
    ...
    hold on
    for j = (1:numel(kL_factor_s))
        plot(alpha, ER_kL2(j,:), DisplayName="kL Factor = " + kL_factor_s(j))
    end
    hold off
    legend
    grid on
...
%% PLOT output - loss - kL 
    ...
    nexttile
    plot(alpha, abs(result_vector_high(1,:,1)).^2, DisplayName="kL Factor = " + kL_factor_s(1))
    hold on
    for j = (2:numel(kL_factor_s))
        plot(alpha, abs(result_vector_high(1,:,j)).^2, DisplayName="kL Factor = " + kL_factor_s(j))
    end
    hold off
    grid on
    legend
    ...
    nexttile
    plot(alpha, abs(result_vector_high(2,:,1)).^2, DisplayName="kL Factor = " + kL_factor_s(1))
    hold on
    for j = (2:numel(kL_factor_s))
        plot(alpha, abs(result_vector_high(2,:,j)).^2, DisplayName="kL Factor = " + kL_factor_s(j))
    end
    hold off
    grid on
    legend
    ...
    nexttile
    plot(alpha, abs(result_vector_low(1,:,1)).^2, DisplayName="kL Factor = " + kL_factor_s(1))
    hold on
    for j = (2:numel(kL_factor_s))
        plot(alpha, abs(result_vector_low(1,:,j)).^2, DisplayName="kL Factor = " + kL_factor_s(j))
    end
    hold off
    grid on
    legend
    ...
    nexttile
    plot(alpha, abs(result_vector_low(2,:,1)).^2, DisplayName="kL Factor = " + kL_factor_s(1))
    hold on
    for j = (2:numel(kL_factor_s))
        plot(alpha, abs(result_vector_low(2,:,j)).^2, DisplayName="kL Factor = " + kL_factor_s(j))
    end
    hold off
    grid on
    legend
...
for j = 1:numel(alpha)
    ...
    for k = 1:numel(kL_factor)
        ...
        result_vector_low = Tc_matrix_c*MC_matrix_1*Tc_matrix_s*[E_i_laser, 0]';
        ...
        result_vector_high = Tc_matrix_c*MC_matrix_2*Tc_matrix_s*[E_i_laser, 0]';
...
tiledlayout(1,2)
    ...
    hold on
    for j=1:numel(alpha)
        plot(kL_factor, ER_a1(j,:), DisplayName="alpha = " + alpha(j) + " [dB/m]")
    end
    hold off
    legend
    grid on
    ...
    hold on
    for j=1:numel(alpha)
        plot(kL_factor, ER_a2(j,:), DisplayName="alpha = " + alpha(j) + " [dB/m]")
    end
    hold off
    grid on
    legend
...
for j = (1: numel(alpha(:, 1)))
    for k = (1: numel(kL_factor(:, 1)))
        ...
        plot(RF, data, DisplayName="alpha = " + alpha(j,1) + "," +alpha(j,2) + " kL = " + kL_factor(k,1) + "," +kL_factor(k,2)) 
    end
end
hold off
grid on
